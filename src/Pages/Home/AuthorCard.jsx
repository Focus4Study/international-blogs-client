...
const AuthorCard = (blog) => {
    const {name, userImg} = blog.blog
    const [subscribed, setSubscribed] = useState(false)
    const handleClick = () =>{
        setSubscribed(!subscribed)
        if (subscribed) {
            toast(`You have Unsubscribed to ${name}`)
        }
        else if (!subscribed) {
            
            toast(`Thank you for subscribing to ${name}`)
        }
    }
            ...
            <div className="flex flex-col justify-center max-w-xs p-6 shadow-md rounded-xl sm:px-12 dark:bg-gray-50 dark:text-gray-800 mx-auto">
                ...
                <div className="space-y-4 text-center divide-y dark:divide-gray-300">
                    <div className="my-2 space-y-1">
                        <h2 className="text-xl font-semibold sm:text-2xl mb-5">{name}</h2>
                        {
                            !subscribed? <button onClick={handleClick} className='btn bg-gray-900 text-white'>Subscribe</button> : <button onClick={handleClick} className='btn btn-primary'>Unsubscribe</button>
                        }
                        <ToastContainer />
                    </div>
